...
# Milestone 0 â€” Freeze the Stable Baseline
...
# Milestone 1 â€” Establish the Session Root
...
# Milestone 2 â€” Define State Ownership and Inspection
...
# Milestone 3 â€” Separate Definitions from Runtime State
...
app/player/
â”œâ”€â”€ character.py
â”œâ”€â”€ player_state.py
â”œâ”€â”€ resources.py
â”œâ”€â”€ progression.py
â”œâ”€â”€ stats.py
â””â”€â”€ loadouts/
    â”œâ”€â”€ branoc.py
    â”œâ”€â”€ azhvielle.py
...
```text
CharacterProfile
â†’ generic archetype instance
â†’ selected profile attached
```
...
# Milestone 4 â€” Formalize Stat and Resource Layers
...
Permanent/base value
    â†“ equipment modifiers
Equipped value
    â†“ temporary modifiers
...
```text
Strength
â†’ physical scaling and stagger pressure

Constitution
â†’ defense and durable techniques

Intelligence
â†’ magic, healing, and mana-related scaling

Dexterity
â†’ ranged attacks, accuracy, initiative, critical behavior

Charisma
â†’ luck, narrative checks, unusual influence mechanics
```
...
# Milestone 5 â€” Create a Shared Combatant Contract
...
```text
Enemy definition
â”œâ”€â”€ starting attributes
â”œâ”€â”€ starting resources
â”œâ”€â”€ move loadout
â””â”€â”€ identity

EnemyState
â”œâ”€â”€ Enemy definition
â”œâ”€â”€ Health
â”œâ”€â”€ Mana
â”œâ”€â”€ effective stats
â””â”€â”€ encounter runtime state
```
...
# Milestone 6 â€” Lock the Move and Resolution Contracts
...
```text
Invalid menu input
â†’ does not consume a turn

Unknown move
â†’ rejected without cost
...
Missed move
â†’ cost remains spent

Current HP and mana
â†’ persist after combat

Temporary combat effects
â†’ clear when combat ends

Equipment
â†’ affects effective values only
```
...
# Milestone 7 â€” Implement the Core Combat Resolver
...
# Milestone 9 â€” Implement Character Combat Identities
...
### 9A-2 â€” State impact declaration
...
I cannot independently fetch tonightâ€™s live GitHub Project README edit through the repository connector. I am therefore treating M9â€™s quoted campaign finish line, encounter compositions, and corrective review as the controlling amendment to the previously retrieved roadmapâ€”not pretending the obsolete progression-only M10 section remains authoritative.

````md
# Dungeon Drifters M10 â€” Playable v0.3 Campaign and Build Week Release
...
```text
select a Drifter
â†’ begin the fixed surface route
â†’ fight multiple authored Goblin encounters
â†’ select exact enemies during multi-enemy combat
â†’ preserve HP, Mana, Super, inventory, equipment, and progression
â†’ receive persistent encounter rewards
â†’ use the fixed rest point
â†’ perform the approved equipment interaction
â†’ save and load at safe campaign boundaries
â†’ defeat the Goblin Lord finale
â†’ enter the dungeon
â†’ recruit the fixed second Drifter
```
...
3. Explicit corrections in M9â€™s review
...
## Lock 1 â€” One GameState per campaign run
...
## Lock 2 â€” Use existing story and world ownership
...
WorldState
â†’ discovered locations
â†’ defeated encounters
â†’ opened or consumed route objects
â†’ persistent dungeon changes
```

Do not duplicate route completion in several unrelated containers.

## Lock 3 â€” Authored route definitions are immutable
...
## Lock 4 â€” Encounter is the campaign reward boundary
...
EncounterDefinition
â†’ encounter ID
â†’ ordered enemy spawn definitions
â†’ authored reward
â†’ route metadata

EncounterState
â†’ runtime enemy instances
â†’ completion state
â†’ encounter-local identifiers
```
...
## Lock 5 â€” Battle remains combat orchestration
...
## Lock 6 â€” Preserve the UI/engine separation
...
## Lock 7 â€” Multi-enemy is player-versus-many only
...
## Lock 8 â€” Exact target identity cannot depend on display names
...
## Lock 9 â€” Persistent resources survive encounters
...
## Lock 10 â€” Rest is the recovery mechanism
...
## Lock 11 â€” Save only at safe campaign boundaries
...
## Lock 12 â€” Evolve persistence once
...
## Lock 13 â€” Reconstruct authored definitions on load
...
```text
saved profile ID
â†’ canonical CharacterProfile
â†’ canonical character factory
â†’ fresh authored loadout
â†’ apply validated mutable saved state
```

## Lock 14 â€” No forced first-Goblin level

EXP pacing must be authored against the complete surface campaign.

Do not inflate the first Goblinâ€™s reward merely to demonstrate leveling.
...
## Lock 15 â€” No unapproved enemy or equipment lore
...
```text
Player phase
â†’ each living enemy acts once in authored order
â†’ repeat
```
...
```text
one living enemy â†’ auto-target
multiple living enemies â†’ explicit target phase
```
...
Mana â†’ full
Super â†’ preserve
prepared payloads â†’ preserve
run inventory â†’ preserve
```
...
# M10-A â€” Campaign Session Shell and Persistent Route State
...
# M10-B â€” Authored Encounter Definitions and Route Progression
...
# M10-C â€” Multi-Enemy Battle, Exact Targeting, and Victory
...
â†’ choose move
â†’ if self-targeted: resolve
â†’ if one enemy target is available: approved auto-target rule
â†’ if multiple enemy targets are available: choose exact target
â†’ validate target against the exact offered view
â†’ resolve canonical actor-owned move
```
...
# M10-D â€” Encounter Rewards and Persistent Progression
...
EncounterReward
â†’ total EXP
â†’ gold, when approved
â†’ fixed item reward ID, when approved
â†’ fixed permanent growth, when approved
```
...
```text
apply secured-EXP scaling from the active Drifterâ€™s effective Intuition
...
# M10-E â€” Minimal Equipment Interaction and Persistence
...
â†’ inspect current equipment
â†’ inspect eligible inventory equipment
â†’ choose slot/item
â†’ equip
â†’ previous item returns to inventory when applicable
â†’ continue route
```
...
# M10-F â€” Fixed Deterministic Rest
...
```text
HP â†’ restore to maximum
Mana â†’ restore to maximum
Super â†’ preserve
prepared payloads â†’ preserve
run inventory â†’ preserve
equipment â†’ preserve
EXP/level â†’ preserve
```
...
# M10-G â€” One-Slot Save and Load
...
```text
read JSON
â†’ validate top-level shape
â†’ validate schema version
â†’ resolve profile IDs
â†’ create canonical characters
â†’ create PlayerState members
â†’ apply validated mutable values
â†’ restore party
â†’ restore StoryState
â†’ restore WorldState
â†’ create GameState
â†’ verify invariants
```
...
# M10-H â€” Goblin Lord Finale, Dungeon Transition, and Recruitment
...
# M10-I â€” Full Regression, Release Hardening, and Submission
...
# Stretch Only â€” First Two-Drifter Dungeon Encounter
...
## Wave 2 â€” Multi-Enemy Foundation
